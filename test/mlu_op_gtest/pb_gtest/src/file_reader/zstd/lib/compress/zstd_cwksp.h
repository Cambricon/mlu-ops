 ...
 * bytes. Used to determine the number of bytes required for a given "aligned".
 */
MEM_STATIC size_t ZSTD_cwksp_aligned_alloc_size(size_t size) {
  return ZSTD_cwksp_alloc_size(
      ZSTD_cwksp_align(size, ZSTD_CWKSP_ALIGNMENT_BYTES));
}
...
MEM_STATIC size_t ZSTD_cwksp_slack_space_required(void) {
   ...
   * 2*ZSTD_CWKSP_ALIGNMENT_BYTES bytes to align the beginning of tables section
   * and end of buffers;
   */
 ...
 * wksp, which counts from the end of the wksp (as opposed to the object/table
 * segment).
 *
 ...
 * ZSTD_CWKSP_ALIGNMENT_BYTES (64 bytes). This memory has been initialized at
 * least once in the past. This doesn't mean it has been initialized this time,
 * and it might contain data from previous operations. The main usage is for
 * algorithms that might need read access into uninitialized memory. The
 * algorithm must maintain safety under these conditions and must make sure it
 * doesn't leak any of the past data (directly or in side channels).
 */
MEM_STATIC void* ZSTD_cwksp_reserve_aligned_init_once(ZSTD_cwksp* ws,
  ...
  if (ptr && ptr < ws->initOnceStart) {
     ...
     * 2. Another initOnce buffer that has been allocated before (and so was
     * previously memset)
     * 3. An ASAN redzone, in which case we don't want to write on it
     * For these reasons it should be fine to not explicitly zero every byte up
     * to ws->initOnceStart. Note that we assume here that MSAN and ASAN cannot
...
MEM_STATIC void* ZSTD_cwksp_reserve_table(ZSTD_cwksp* ws, size_t bytes) {
  ...
  /* We can only start allocating tables after we are done reserving space for
   * objects at the start of the workspace */
  if (ws->phase < phase) {
    if (ZSTD_isError(ZSTD_cwksp_internal_advance_phase(ws, phase))) {
      return NULL;
    }
...
MEM_STATIC void ZSTD_cwksp_mark_tables_dirty(ZSTD_cwksp* ws) {
   ...
   * the initOnce portion as it break its promise. This means that this
   * poisoning check isn't always applied fully. */
  {
 ...
 * limit of the actual amount of space used.
 */
MEM_STATIC int ZSTD_cwksp_estimated_space_within_bounds(
    const ZSTD_cwksp* const ws, size_t const estimatedSpace) {
  /* We have an alignment space between objects and tables between tables and
   * buffers, so we can have up to twice the alignment bytes difference between
   * estimation and actual usage */
