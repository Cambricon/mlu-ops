...
namespace mluoptest {
  ...
  void Executor::checkBaseline() {
    ...
    if (is_get_base_data) {
      ...
      if (!is_baseline_pass) {
        eva_res_.what.emplace_back("The performance result exceed baseline "
                                   "threshold.");
      }
      ...
      if (quant_mode == NO_QUANT) {
        ...
        if (dst_dtype == MLUOP_DTYPE_INT8) {
          MLUOP_CHECK(castFloat32ToFixed(
              src_data, (int8_t *)dst_data, count, *pos, *sc, *off));
          if (dequantify) {
            MLUOP_CHECK(castFixedToFloat32(
                (int8_t *)dst_data, src_data, count, *pos, *sc, *off));
          }
        } else if (dst_dtype == MLUOP_DTYPE_INT16) {
          MLUOP_CHECK(castFloat32ToFixed(
              src_data, (int16_t *)dst_data, count, *pos, *sc, *off));
          if (dequantify) {
            MLUOP_CHECK(castFixedToFloat32(
                (int16_t *)dst_data, src_data, count, *pos, *sc, *off));
          }
        }
      }
    } else if ((src_dtype == MLUOP_DTYPE_FLOAT &&
    ...
    if (src_dtype == dst_dtype) {
      ...
      if (flag_quant_mode_ == NO_QUANT) {
        ...
        if (src_dtype == MLUOP_DTYPE_INT8) {
          MLUOP_CHECK(castFixedToFloat32(
              (int8_t *)src_data, dst_data, count, pos, sc, off));
        } else if (src_dtype == MLUOP_DTYPE_INT16) {
          MLUOP_CHECK(castFixedToFloat32(
              (int16_t *)src_data, dst_data, count, pos, sc, off));
        }
      }
