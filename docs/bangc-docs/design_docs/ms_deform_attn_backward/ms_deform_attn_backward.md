...
| 审批人/日期 | 张少鹏/2022-10-24                                            |
| 审批人/日期 | 王远/2022-10-24                                          |
| 审批人/日期 | 卜德飞/2022-10-24                                       |
...
本文档为`ms_deform_attn_backward`算子的设计文档，包括需求分析、接口设计、方案设计和方案实施部分。
...
| 是否需要支持stride机制  | 否                                                  |
| 是否需要支持广播  | 否                       |
| 0元素检查是否直接返回  | 是（返回CNNL_STATUS_BAD_PARAM）                                                  |
...
| ----------- | ------------------ | ----------------- | ------------ | -------- | -------- |
| handle      | CNNL 句柄，保存运行的上下文信息。   | 输入              | mluOpHandle_t | /        | /       |
