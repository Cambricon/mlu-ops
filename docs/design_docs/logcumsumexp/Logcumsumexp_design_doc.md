...
- 如果dim = 0，沿着维度0，则计算<img src="equation_4.png" style="zoom:33%;" />，结果为 5.018，存入与5对应的位置。
...
第三种情况，当目标维度为张量的最高维度时，我们把输入数据按若干列分块，然后将每一块通过2D拷贝拷到一个NRAM上，每个NRAM计算逐行累加，若NRAM容量不足以处理整块则分多次加载该块，计算出结果后在通过2D拷贝将数据重新加载到GDRAM上。

<img src="HighestDim.png" style="zoom:50%;" />
