...
typedef enum {
  ...
  /*!< MLUOP library was not initialized properly, which is usually caused by
     the failure of calling ::mluOpCreate, ::mluOpCreateTensorDescriptor or
     ::mluOpSetTensorDescriptor.
       Such error is usually due to incompatible MLU device or invalid driver
     environment.
       Notice that ::mluOpCreate should be called prior to any other mluOp
     functions.*/
  MLUOP_STATUS_ALLOC_FAILED = 2,
  /*!< This error occurs when the resource allocation failed, usually caused by
     the failure of cnMallocHost, probably because of the exceeded memory usage.
     Please make sure that the memory allocated previously is deallocated as
     much as possible.*/
  MLUOP_STATUS_BAD_PARAM = 3,
  /*!< Invalid value or parameters passed to the function, including data type,
     layout, dimensions, etc.*/
  MLUOP_STATUS_INTERNAL_ERROR = 4,
  /*!< Error occurred inside of the function, which may indicate an internal
     error or bug in the library. This error is usually due to the failure of
     cnrtMemcpyAsync. Please check whether the memory passed to the function was
  ...
  /*!< Error occurred when the function failed to execute on MLU device due to
     multiple reasons. You can check whether the hardware environment, driver
     version and other prerequisite libraries are correctly installed. For more
     information about prerequisite libraries, see "Cambricon MLUOP User
     Guide".*/
  ...
  /*!< Numerical overflow occurred when executing the function,
       which is usually due to large scale or inappropriate range of value of
     input tensor.*/
} mluOpStatus_t;
...
typedef enum {
  MLUOP_LAYOUT_NCHW = 0,
  /*!< The data layout is in the following order: batch size, channel, height,
     and width. */
  MLUOP_LAYOUT_NHWC = 1,
  /*!< The data layout is in the following order: batch size, height, width, and
     channel. */
  MLUOP_LAYOUT_HWCN = 2,
  /*!< The data layout is in the following order: height, width, channel and
     batch size. */
  MLUOP_LAYOUT_NDHWC = 3,
  /*!< The data layout is in the following order: batch size, depth, height,
     width, and channel.*/
  MLUOP_LAYOUT_ARRAY = 4,
  /*!< The data is multi-dimensional tensor. */
  MLUOP_LAYOUT_NCDHW = 5,
  /*!< The data layout is in the following order: batch size, channel, depth,
     height, and width.*/
  MLUOP_LAYOUT_TNC = 6,
  /*!< The data layout is in the following order: timing steps, batch size,
     alphabet size.*/
  MLUOP_LAYOUT_NTC = 7,
  /*!< The data layout is in the following order: batch size, timing steps,
     alphabet size.*/
  MLUOP_LAYOUT_NC = 8,
...
typedef enum {
  ...
  MLUOP_DTYPE_UINT8 =
      8, /*!< The data is an 8-bit unsigned integer data type. */
  MLUOP_DTYPE_UINT16 =
      9, /*!< The data is a 16-bit unsigned integer data type. */
  MLUOP_DTYPE_UINT32 =
      10, /*!< The data is a 32-bit unsigned integer data type. */
  MLUOP_DTYPE_UINT64 =
      11, /*!< The data is a 64-bit unsigned integer data type. */
  MLUOP_DTYPE_BOOL = 12, /*!< The data is a boolean data type. */
  MLUOP_DTYPE_COMPLEX_HALF =
      13, /*!< The data is a 32-bit complex number of two fp16. */
  MLUOP_DTYPE_COMPLEX_FLOAT =
      14, /*!< The data is a 64-bit complex number of two fp32. */
} mluOpDataType_t;
...
typedef enum {
  ...
  /*!< Implementation with the high-precision algorithm regardless the
     performance.*/
...
/*! The descriptor of the collection of tensor which is used in the RNN
 * operation, such as weight, bias, etc. You need to call the
 * ::mluOpCreateTensorSetDescriptor function to create a descriptor, and call
 * the ::mluOpInitTensorSetMemberDescriptor to set the information about each
 * tensor in the tensor set. If the data type of the tensor in the tensor set is
 * in fixed-point data type, call
 * ::mluOpInitTensorSetMemberDescriptorPositionAndScale function to set
 * quantization parameters. At last, you need to destroy the descriptor at the
 * end with the
...
/*!
 *  @brief Initializes the MLUOP library and creates a handle \b handle to a
 * structure that holds the MLUOP library context. It allocates hardware
 * resources on the host and device. You need to call this function before any
 * other MLUOP functions.
 *
 *  You need to call the ::mluOpDestroy function to release the resources later.
 *
 *  @param[out] handle
 *    Output. Pointer to the MLUOP context that is used to manage MLU devices
 * and queues. For detailed information, see ::mluOpHandle_t.
 *  @par Return
...
/*!
 *  @brief Updates the MLUOP context information that holds by the \b handle.
 * This function should be called if you call Cambriocn Driver API
 * cnSetCtxConfigParam to set the context information. The related context
 * information will be synchronized to MLUOP with this function. For detailed
 * information, see Cambricon Driver API Developer Guide.
 *
 *  @param[in] handle
...
/*!
 *  @brief Releases the resources of the specified MLUOP handle \b handle that
 * was created by the ::mluOpCreate function. It is usually the last call to
 * destroy the handle to the MLUOP handle.
 *
 *  @param[in] handle
...
/*!
 *  @brief Sets the runtime queue \b queue in the handle \b handle. The queue is
 * used to launch kernels or to synchronize to this queue.
 *
 *  Before setting a queue \b queue, you need to call the ::mluOpCreate function
 * to initialize MLUOP library, and call the cnrtCreateQueue function to create
 * a queue \b queue.
 *
 *  @param[in] handle
...
/*!
 *  @brief Retrieves the queue \b queue that was previously set to the handle \b
 * handle.
 *
 *  @param[in] handle
 *    Input. Handle to a MLUOP context that is used to manage MLU devices and
 *    queues. For detailed information, see ::mluOpHandle_t.
 *  @param[out] queue
 *    Output. Pointer to the queue that was previously set to the specified
 * handle.
 *  @par Return
 *  - ::MLUOP_STATUS_SUCCESS, ::MLUOP_STATUS_BAD_PARAM
 *
...
/*!
 *  @brief Converts the MLUOP enumerated status code to ASCIIZ static string and
 * returns a pointer to the MLU memory that holds information about ASCIIZ
 * static string with the status name. For example, when the input argument is
 *  ::MLUOP_STATUS_SUCCESS, the returned string is MLUOP_STATUS_SUCCESS. When an
 * invalid status value is passed to the function, the returned string is
 ...
 *
 *  You need to call the ::mluOpCreateTensorDescriptor function to create a
 * descriptor, and call the ::mluOpSetTensorDescriptor function or the
 * ::mluOpSetTensorDescriptorEx function to set the tensor information to the
 * descriptor. Also, you need to destroy the MLUOP context at the end with the
 * ::mluOpDestroyTensorDescriptor function.
 */
// Group:QuantizeRoundMode
/*!
 *  @brief Updates the specific rounding mode of MLUOP context information that
 * holds by the \b handle. This function should be called if you want to change
 * the mluop rounding mode that used to cumulate the results. For detailed
 * information, see Cambricon Driver API Developer Guide.
 *
 *  @param[in] handle
 *    Input. Pointer to the MLUOP context that is used to manage MLU devices and
 *    queues. For detailed information, see ::mluopHandle_t.
 *  @param[in] round_mode
 *    Input. The rounding mode of quantization conversion to be set to the MLUOP
 * handle.
 *  @par Return
 *  - ::MLUOP_STATUS_SUCCESS, ::MLUOP_STATUS_BAD_PARAM
 *
 *  @note
 *  - On MLU200 series:
 *    You can't set MLUOP_ROUND_HALF_TO_EVEN for the rounding mode because the
 ...
 *    Output. the rounding mode of quantization conversion that was previously
 * set to the specified handle.
 *
 *  @par Return
 *  - ::MLUOP_STATUS_SUCCESS, ::MLUOP_STATUS_BAD_PARAM
 *
 *  @note
 *  - The default round mode of default initialized mluopHandle_t is
 * MLUOP_ROUND_TO_EVEN.
 *
 *  @par Requirements
 *  - None.
...
/*!
 *  @brief Creates a tensor descriptor pointed by \b desc that holds the
 * dimensions, data type, and layout of input tensor. If the input tensor is in
 * fixed-point data type, the ::mluOpSetTensorDescriptorPositionAndScale
 * function or the
 * ::mluOpSetTensorDescriptorPosition
 *  function needs to be called to set quantization parameters.
 *
 *  The ::mluOpDestroyTensorDescriptor function needs to be called to destroy
 * the tensor descriptor later.
 *
 *  @param[in] desc
 *    Input. Pointer to the struct that holds information about the tensor
 * descriptor.
 *  @par Return
 *  - ::MLUOP_STATUS_SUCCESS, ::MLUOP_STATUS_BAD_PARAM
 *
 ...
 * tensor.
 *
 *  If ::mluOpSetTensorDescriptor is called, you do not need to specify the
 * strides of all dimensions. The strides are inferred by parameters passed to
 * this function. Also, the data will be treated as contiguous in memory with no
 * padding between dimensions. To specify the strides of all dimensions, you can
 * call ::mluOpSetTensorDescriptorEx. But the data might not be treated as
 ...
 *  @param[in] layout
 *    Input. The layout of the input tensor. For detailed information, see
 * ::mluOpTensorLayout_t.
 *  @param[in] dtype
 *    Input. The data type of the input tensor. For detailed information, see
 * ::mluOpDataType_t.
 *  @param[in] dimNb
 *    Input. The number of dimensions in the input tensor of the initialized
 * operation.
 *  @param[in] dimSize
 ...
 *
 *  @note
 *  - dimSize[0] represents the highest dimension, dimSize[DIM_MAX - 1]
 * represents the lowest dimension, and DIM_MAX represents the number of
 * dimensions in the input tensor.
 *  - This function cannot be called continuously. You need to call
 * ::mluOpResetTensorDescriptor before calling another
 * ::mluOpSetTensorDescriptor to avoid memory leaks.
 *
 *  @par Requirements
 *  - None.
 *
 ...
 *
 *  If ::mluOpResetTensorDescriptor is called, all the information about the
 * tensor will be reset to initial value, which means layout is
 * MLUOP_LAYOUT_ARRAY, dtype is MLUOP_DTYPE_FLOAT, dimsNb is 0, and dimSize
 * points to an \b MLUOP_DIM_MAX-dimension array.
 *
 *  @param[in] desc
 *    Input. The descriptor of the tensor. For detailed information, see
 * ::mluOpTensorDescriptor_t.
 *  @par Return
 *  - ::MLUOP_STATUS_SUCCESS, ::MLUOP_STATUS_BAD_PARAM
 *
 *  @note
 *  - This function is used to avoid memory leaks when more than one
 * ::mluOpSetTensorDescriptor function is called. You should call this function
 ...
 *  @brief Initializes the tensor descriptor pointed by \b desc that is
 * previously created with the ::mluOpCreateTensorDescriptor function, and sets
 * the information about the dimensions, strides, data type, and layout of the
 * input tensor.
 *
 *  Compare with ::mluOpSetTensorDescriptor, you can specify the strides of all
 * dimensions with this function. If ::mluOpSetTensorDescriptor is called, you
 * do not need to specify the strides of all dimensions and the strides are
 * inferred by parameters passed to this function.
 *
 *  This function does not support all the operations in this version. You can
 * check if an operation supports this function in the "note" section of the
 ...
 *
 *  @note
 *  - dimSize[0] represents the highest dimension, and dimSize[DIM_MAX - 1]
 ...
 *  The on-chip data type \b onchip_dtype can be different from the off-chip
 * data type of the tensor. This function is optional. If the on-chip data type
 * is not set with this function, the
 ...
 *    Input. The on-chip data type of the tensor used in the operations that
 * support fixed-point computing.
 *  @par Return
 *  - ::MLUOP_STATUS_SUCCESS, ::MLUOP_STATUS_BAD_PARAM
 *
 *  @note
 *  - The on-chip data type is only used on the operations that support
 * fixed-point computing. It has no effect on other operations. If you call this
 * function to get on-chip data type for an operation that does not support
 * fixed-point computing, ::MLUOP_STATUS_BAD_PARAM is returned. To check if an
 * operation supports fixed-point computing, see the detailed description of the
 * operation.
 *
 *  @par Requirements
 *  - None.
...
/*!
 * @brief Sets the \b position factor to the descriptor \b desc of fixed-point
 * data in fixed-point quantization. It is used in ::MLUOP_QUANTIZE_POSITION
 * mode.
 *
 * @param[in] desc
...
/*!
 * @brief Sets the \b position and \b scale factors to the descriptor of
 * fixed-point data in fixed-point quantization. It is used in
 * ::MLUOP_QUANTIZE_POSITION_SCALE mode.
 *
 * @param[in] desc
 *   Input. The descriptor of the tensor. For detailed information,
 *   see ::mluOpTensorDescriptor_t.
...
/*!
 * @brief Sets the \b position, \b scale and \b offset factors to the descriptor
 * of fixed-point data in fixed-point quantization. It is used in
 * ::MLUOP_QUANTIZE_POSITION_SCALE_OFFSET mode.
 *
 * @param[in] desc
 *   Input. The descriptor of the tensor. For detailed information,
 *   see ::mluOpTensorDescriptor_t.
...
/*!
 *  @brief Retrieves a tensor descriptor \b desc that is previously created with
 * the
 *  ::mluOpCreateTensorDescriptor function, and sets the information about the
 * dimensions, data type, and layout of input tensor.
 *
 *  @param[in] desc
 *    Input. The descriptor of the input tensor. For detailed information,
 *    see ::mluOpTensorDescriptor_t.
 *  @param[out] layout
 *    Output. Pointer to the host memory that holds information about the layout
 * of the input tensor. For detailed information, see ::mluOpTensorLayout_t.
 *  @param[out] dtype
 *    Output. Pointer to the host memory that holds information about the data
 * type of the input tensor. For detailed information, see ::mluOpDataType_t.
 *  @param[out] dimNb
 *    Output. Pointer to the host memory that holds information about the
 ...
 * data type, stride and layout of input tensor with
 ...
 *    Output. An array that contains the size of the tensor for each dimension.
 *  @param[out] dimStride
 *    Output. An array that contains the stride of the tensor for each
 * dimension.
 *  @par Return
 *  - ::MLUOP_STATUS_SUCCESS, ::MLUOP_STATUS_BAD_PARAM
 *
...
/*!
 *  @brief Retrieves the number of elements according to the input descriptor \b
 desc. You
 *  need to call the ::mluOpSetTensorDescriptor function first to create a
 tensor descriptor
 ...
 *  @par Example
     @verbatim
      ...
      mluOpSetTensorDescriptor(input_desc, MLUOP_LAYOUT_ARRAY,MLUOP_DTYPE_FLOAT,
 2,{2, 3}); size_t nums=mluOpGetTensorElementNum(input_desc);  // nums = 6
 ...
 *  If the on-chip data type is not set with the
 * ::mluOpSetTensorDescriptorOnchipDataType function, the
 ...
 *    see ::mluOpTensorDescriptor_t.
 *  @param[in] onchip_dtype
 *    Input. Pointer to the MLU memory that holds information about the on-chip
 ...
 *  - The on-chip data type is only used on the operations that support
 * fixed-point computing. It has no effect on other operations. If you call this
 * function to get on-chip data type for an operation that does support
 * fixed-point computing, ::MLUOP_STATUS_BAD_PARAM is returned. To check if an
 * operation supports fixed-point computing, see the detailed description of the
 * operation.
 *
...
/*!
 * @brief Gets the \b position factor to the descriptor \b desc of fixed-point
 ...
 *   see ::mluOpTensorDescriptor_t.
 * @param[out] position
 *   Output. A host pointer of fixed position factor that is used for
 * quantization.
 * @par Return
 * - ::MLUOP_STATUS_SUCCESS, ::MLUOP_STATUS_BAD_PARAM
 *
 ...
 *
 */
mluOpStatus_t MLUOP_WIN_API mluOpGetTensorDescriptorPosition(
    const mluOpTensorDescriptor_t desc, int *position);

/*!
 *  @brief Gets the position and scale factors of a tensor descriptor \b desc
 ...
 *    see ::mluOpTensorDescriptor_t.
 *  @param[out] position
 *    Output. Pointer to the MLU memory that holds information about fixed
 * position used for quantization.
 *  @param[out] scale
 *    Output. Pointer to the MLU memory that holds information about scale
...
/*!
 * @brief Gets the \b position, \b scale and \b offset factors to the descriptor
 * \b desc of fixed-point data in fixed-point quantization.
 *
 * @param[in] desc
 *   Input. The descriptor of the tensor. For detailed information,
 *   see ::mluOpTensorDescriptor_t.
 ...
 * created with the
 *  ::mluOpCreateTensorSetDescriptor function.
 *
...
mluOpStatus_t MLUOP_WIN_API mluOpInitTensorSetMemberDescriptorPositionAndScale(
    mluOpTensorSetDescriptor_t tensorSetDesc,
    const int setDimNb,
    const int tensorIndex[],
    const int position,
    const float scale);
