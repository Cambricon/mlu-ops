...
class CosineEmbeddingLoss(object):
    ...
    def __init__(self, dtype, stage, target, task_type):
        ...
        self.input_y = self.tcp.Buffer(
            shape = (self.data_num_v, ),
            name = "input_y",
            dtype = dtype,
            scope = "global")
        self.margin = self.tcp.Var(
            name = "margin",
    ...
    def compute_body(self):
        ...
        kernel_size = self.tcp.Scalar(
            bangpy.int32,
            name = "kernel_size")
        kernels_nram = self.tcp.Scalar(
            bangpy.int32,
            name = "kernels_nram")
        kernels_per_line = self.tcp.Scalar(
            bangpy.int32,
            name = "kernels_per_line")
        kernels_per_row = self.tcp.Scalar(
            bangpy.int32,
            name = "kernels_per_row")
        lines_per_row = self.tcp.Scalar(
            bangpy.int32,
            name = "lines_per_row")
        rows_per_line = self.tcp.Scalar(
            bangpy.int32,
            name = "rows_per_line")

        # batch指nram一次能处理的row数量，如果batch == 0则表示需要多一层循环处理这个row
        batch_size = self.tcp.Scalar(
            bangpy.int32,
            name = "batch_size")
        # nram中存储的数据的row数量
        y_num = self.tcp.Scalar(
            bangpy.int32,
            name = "y_num")
        ...
        # 源数据一行所需内存行数
        lines_per_row.assign((kernels_per_row + kernels_per_line - 1) // kernels_per_line)

         # 内存中一行数据能够存储的源数据行数
        rows_per_line.assign(kernels_per_line // kernels_per_row)

        # 当一行数据装不满一整个buffer时计算这一行数据需要的kernel数量
        kernels_per_line_n = self.tcp.Scalar(dtype = bangpy.int32, name = "kernels_per_line_n")
        kernels_per_line_n.assign((kernels_per_row + self.align_size - 1) // self.align_size)
        ...
        task_row_num = self.tcp.Scalar(
            bangpy.int32,
            name = "cluster_row_num")
        task_row_num.assign(self.data_num // self.task_num)

        task_row_base = self.tcp.Scalar(
            bangpy.int32,
            name = "task_row_base")
        with self.tcp.if_scope(self.data_num - task_row_num * self.task_num > self.tcp.taskId):
            task_row_num.assign(task_row_num + 1)
            task_row_base.assign(task_row_num * self.tcp.taskId)
        with self.tcp.else_scope():
            task_row_base.assign(self.data_num - (self.task_num - self.tcp.taskId) * task_row_num)

        task_row_end = self.tcp.Scalar(
            bangpy.int32,
            name = "task_row_end")
        ...
        # 这样是为了在内层循环设置流水线
        inner_loop_bound = self.tcp.Scalar(
            bangpy.int32,
            name = "inner_loop_bound")
        outer_loop_bound = self.tcp.Scalar(
            bangpy.int32,
            name = "outer_loop_bound",
            value = 1)

        with self.tcp.if_scope(batch_size == 0):
            inner_loop_bound.assign((kernels_per_row + kernels_nram - 1) // kernels_nram)
        ...
        with self.tcp.for_range(0, outer_loop_bound) as i:
            ...
            lower1_sum = self.tcp.Scalar(name = "lower1_sum", dtype = bangpy.float32, value = 0)
            lower2_sum = self.tcp.Scalar(name = "lower2_sum", dtype = bangpy.float32, value = 0)
            ...
            with self.tcp.for_range(0, inner_loop_bound, stage = self.pipeline) as j:
                ...
                input_buffer_y = self.tcp.Buffer(
                    ...
                    dtype = self.dtype,
                    scope = "nram")\
                        [:(batch_size + self.align_size - 1) // self.align_size * self.align_size]
                ...
                with self.tcp.block("data_copy"):
                    ...
                    with self.tcp.if_scope(batch_size == 0):
                        ...
                        self.tcp.memcpy(
                            ...
                            self.input_x1[row][base : end])
                        self.tcp.memcpy(
                            input_buffer_x2[:end - base],
                            self.input_x2[row][base : end])
                    ...
                    with self.tcp.elif_scope(rows_per_line ==  0):
                        ...
                        self.tcp.memcpy(input_buffer_y[:end - base], self.input_y[base:end])

                        cpy_in_x11 = input_buffer_x1[:batch_size * self.length]\
                            .reshape((batch_size, self.length))
                        cpy_in_x21 = input_buffer_x2[:batch_size * self.length]\
                            .reshape((batch_size, self.length))
                        self.tcp.memcpy(cpy_in_x11[:end - base], self.input_x1[base : end])
                        self.tcp.memcpy(cpy_in_x21[:end - base], self.input_x2[base : end])
                    ...
                    with self.tcp.else_scope():
                        ...
                        # 内存错位拷贝
                        cpy_in_x1 = input_buffer_x2[:batch_size * self.length]\
                            .reshape((batch_size, self.length))
                        cpy_in_x2 = inter_buffer[:batch_size * self.length]\
                            .reshape((batch_size, self.length))
                        self.tcp.memcpy(cpy_in_x1[:end - base], self.input_x1[base : end])
                        self.tcp.memcpy(cpy_in_x2[:end - base], self.input_x2[base : end])

                with self.tcp.block("compute"):
                    # 计算和数据拷入时一样，也依据数据尺寸不同分三种逻辑
                    # nram装不下一行数据
                    with self.tcp.if_scope(batch_size == 0):
                        inter_n = inter_buffer[:kernel_size * kernels_nram]\
                            .reshape((kernels_per_line * kernel_size, self.align_size))
                        temp_n = temp_buffer[:kernels_per_line * self.align_size]\
                        ...
                        compute_sum_batch_0(input_buffer_x1, input_buffer_x1, lower1_sum)
                        compute_sum_batch_0(input_buffer_x2, input_buffer_x2, lower2_sum)

                    with self.tcp.else_scope():
                        # nram能装不到align_size行数据
                        with self.tcp.if_scope(rows_per_line == 0):
                            comps_x1 = input_buffer_x1[: batch_size * self.length].reshape((batch_size, self.length))
                            comps_x2 = input_buffer_x2[: batch_size * self.length].reshape((batch_size, self.length))

                            comp_inter = inter_buffer[:kernels_per_line_n * self.align_size * kernel_size].reshape((kernel_size * kernels_per_line_n, self.align_size, ))
                            temp_n = temp_buffer[:kernels_per_line_n * self.align_size].reshape((kernels_per_line_n, self.align_size))
                            ...
                            with self.tcp.for_range(0, batch_size) as k:
                                ...
                                upper[k] = ((input_buffer_y[k] + 1) * (1 - upper_sum) +\
                                     (1 - input_buffer_y[k]) * self.tcp.scalar_max(lower1_sum, lower2_sum)) / 2

                        # nram能装下不止align_size行数据
                        with self.tcp.else_scope():
                            comps_x1 = input_buffer_x1[: self.length * batch_size]\
                                .reshape((self.length, batch_size))
                            comps_x2 = input_buffer_x2[: self.length * batch_size]\
                                .reshape((self.length, batch_size))
                            comps_inter = inter_buffer[: self.length * batch_size]\
                                .reshape((self.length, batch_size))
                            ...
                            temp_n = temp_buffer[:kernels_per_row * batch_size].reshape((kernels_per_row, batch_size))
                            with self.tcp.if_scope(j <= 1):
                                self.tcp.assign(inter_buffer, 0)
            ...
            with self.tcp.if_scope(batch_size == 0):
                ...
                lower1_sum.assign(0)
                lower2_sum.assign(upper_sum - self.margin)
                self.output[row] = ((self.input_y[row] + 1) * (1 - upper_sum) + (1 - self.input_y[row])\
                    * self.tcp.scalar_max(lower1_sum, lower2_sum)) / 2
