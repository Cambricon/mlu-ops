...
更多说明可见http://wiki.cambricon.com/pages/viewpage.action?pageId=76996704, http://wiki.cambricon.com/pages/viewpage.action?pageId=126459923
## 3 Usage Example
...
```
python3 ./tools/Perf_Analyse/analyse_compile_time.py  --log-path /DEV_SOFT_TRAIN/jiaminghao/compile.log  --working-directory $PWD/build
```
...
```bash
python3 so_analyser.py --so_path libcnnl_0.so
```

#### 对比两个libcnnl*.so*文件的大小

```bash
python3 so_analyser.py --so_path libcnnl_0.so --so_path_compare libcnnl_1.so
```
