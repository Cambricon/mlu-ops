...
typedef enum {
  CNFFT_FUNC_MATMUL = 0,  // directly matmul strategy, specified for multiple batches of transform,
                          // and output size is relatively small. Its structure is suitable tensor
                          // computing-oriented machines.
  CNFFT_FUNC_STOCKHAM = 1,   // an iterative FFT algorithm for n = r^l. It is self-sorting (does not
                             // have a digit reversal permutation). Its structure is suitable for
                             // long vector computing machines.
  CNFFT_FUNC_FOUR_STEP = 2,  // a recursive FFT algorithm for n = km. It is built from two stages of
                             // vector FFTs, the twiddle diagonal and a transposition. Its structure
                             // is suitable for vector computers.
  CNFFT_FUNC_BLUESTEIN = 3,  // a general-purpose algorithm (i.e., n is a prime number).

  CNFFT_FUNC_COOLEY_TUKEY = 4,  // a recursive FFT algorithm for n = 2^m * L; It saves the space
...
struct mluOpFFTStruct {
  ...
  int inembed[FFT_DIM_MAX];  // Pointer of size rank that indicates the storage dimensions of the
                             // input data in memory.
  int inum;                  // element num of input tensor
  int istride;  // distance between two successive input elements in the innermost dimension
  int idist;    // distance between the first element of two consecutive signals in a batch of the
                // input data
  int odim;     // the dimension size of output tensor
  int onembed[FFT_DIM_MAX];  // Pointer of size rank that indicates the storage dimensions of the
                             // output data in memory
  int onum;                  // element num of output tensor
  int ostride;  // distance between two successive output elements in the innermost dimension
  int odist;    // distance between the first element of two consecutive signals in a batch of the
                // output data
  int batch;    // batch size for this transform
  int L;        // n = L * 2^m, L size for this transform
  int m;        // n = L * 2^m, m size for this transform
  int s;        // The size that can be put down on NRAM: L * 2^s, only used by Cooley-Tukey
                // algorithm
  int L_sub;    // The size that can be put down on NRAM: L_sub * 2^m, only used by  Stockham
                // algorithm
...
struct AddrNode {
  ...
  DT* w_tmp2;
  DT* w_tmp3;
  // temp addr reserved for subgraph internal merge calculation, using the same addr with w_tmp*.
  DT* wz_rr;
  DT* wz_ri;
  DT* wz_ir;
  DT* wz_ii;
...
mluOpStatus_t selectStrategy(mluOpHandle_t handle,
