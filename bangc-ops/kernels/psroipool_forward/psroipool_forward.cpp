...
static mluOpStatus_t paramCheck(
    ...
    const float spatial_scale, const int group_size, const void *input,
    const void *rois, const void *output, const void *mapping_channel,
  ...
  // channels == output_h * output_w * output_c
...
mluOpStatus_t MLUOP_WIN_API mluOpPsRoiPoolForward(
  ...
  mluOpStatus_t ret = paramCheck(api, handle, spatial_scale, group_size, input,
                                 rois, output, mapping_channel, input_desc,
                                 rois_desc, output_desc, mapping_channel_desc);
  ...
  if (MLUOP_GEN_CASE_ON_NEW) {
    ...
    GEN_CASE_OP_PARAM_SINGLE(0, "psroipool_forward", "spatial_scale",
                             spatial_scale);
    GEN_CASE_OP_PARAM_SINGLE(1, "psroipool_forward", "group_size", group_size);
  ...
  KERNEL_CHECK((mluOpBlockKernelPsRoiPoolForwardFloat(
      ...
      batch_size, height, width, channels, output_h, output_w, output_c,
      group_size, rois_sum, rois_offset, spatial_scale)));
  GEN_CASE_END();
  return MLUOP_STATUS_SUCCESS;
}
