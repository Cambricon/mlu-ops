...
本文档为`three_interpolate_forward`算子的设计文档，包括需求分析、接口设计、方案设计、性能优化记录。
...
| weights      |   表示输出特征上三个最近邻特征的加权系数，shape是[B, N, 3], B是batch大小，N表示一个输出channel上特征点的个数，3表示有三个最近邻特征加权系数   | 输入              | half, float | ARRAY     | 无       |
