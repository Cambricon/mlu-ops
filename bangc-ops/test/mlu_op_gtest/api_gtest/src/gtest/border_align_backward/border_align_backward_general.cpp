...
class border_align_backward_general : public testing::TestWithParam<BorderAlignBackwardParam> {
 public:
  void SetUp() {
    try {
      ...
      MLUOP_CHECK(mluOpSetTensorDescriptor(
          grad_output_desc_, grad_output_params.get_layout(), grad_output_params.get_dtype(),
          grad_output_params.get_dim_nb(), grad_output_params.get_dim_size().data()));
      if (mluOpGetTensorElementNum(grad_output_desc_) >= LARGE_TENSOR_NUM) {
        GTEST_CHECK(CNRT_RET_SUCCESS ==
        cnrtMalloc(&grad_output_, 2 * 16));
      } else {
        GTEST_CHECK(CNRT_RET_SUCCESS ==
        cnrtMalloc(&grad_output_, mluOpDataTypeBytes(grad_output_params.get_dtype()) *
      ...
      MLUOP_CHECK(mluOpSetTensorDescriptor(boxes_desc_, boxes_params.get_layout(),
                                         boxes_params.get_dtype(), boxes_params.get_dim_nb(),
                                         boxes_params.get_dim_size().data()));
      if (mluOpGetTensorElementNum(boxes_desc_) >= LARGE_TENSOR_NUM) {
        GTEST_CHECK(CNRT_RET_SUCCESS ==
        cnrtMalloc(&boxes_, 2 * 16));
      } else {
        GTEST_CHECK(CNRT_RET_SUCCESS ==
      ...
      MLUOP_CHECK(mluOpSetTensorDescriptor(argmax_idx_desc_, argmax_idx_params.get_layout(),
      ...
      if (mluOpGetTensorElementNum(argmax_idx_desc_) >= LARGE_TENSOR_NUM) {
        GTEST_CHECK(CNRT_RET_SUCCESS ==
        cnrtMalloc(&argmax_idx_, (2 * 16)));
      } else {
        GTEST_CHECK(CNRT_RET_SUCCESS ==
        cnrtMalloc(&argmax_idx_, mluOpDataTypeBytes(argmax_idx_params.get_dtype()) *
      ...
      MLUOP_CHECK(mluOpSetTensorDescriptor(grad_input_desc_, grad_input_params.get_layout(),
      ...
      if (mluOpGetTensorElementNum(grad_input_desc_) >= LARGE_TENSOR_NUM) {
        GTEST_CHECK(CNRT_RET_SUCCESS ==
        cnrtMalloc(&grad_input_, 2 * 16));
      } else {
        GTEST_CHECK(CNRT_RET_SUCCESS ==
        cnrtMalloc(&grad_input_, mluOpDataTypeBytes(grad_input_params.get_dtype()) *
      ...
      FAIL() << "MLUOPAPIGTEST: catched " << e.what() << " in border_align_backward_general";
    }
  }
...
TEST_P(border_align_backward_general, negative) {
  EXPECT_TRUE(compute());
}

INSTANTIATE_TEST_CASE_P(
    zero_element_0,
    border_align_backward_general,
    testing::Combine(testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                                       MLUOP_DTYPE_FLOAT,
                                                       4,
                                                       std::vector<int>({0, 4, 4, 1}))),
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_ARRAY,
                                                       MLUOP_DTYPE_FLOAT,
                                                       3,
                                                       std::vector<int>({0, 4, 4}))),
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                                       MLUOP_DTYPE_INT32,
                                                       4,
                                                       std::vector<int>({0, 4, 4, 1}))),
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
...
INSTANTIATE_TEST_CASE_P(
    ...
    testing::Combine(testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                                       MLUOP_DTYPE_FLOAT,
                                                       4,
                                                       std::vector<int>({1, 0, 4, 1}))),
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_ARRAY,
                                                       MLUOP_DTYPE_FLOAT,
                                                       3,
                                                       std::vector<int>({1, 0, 4}))),
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                                       MLUOP_DTYPE_INT32,
                                                       4,
                                                       std::vector<int>({1, 0, 4, 1}))),
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                                       MLUOP_DTYPE_FLOAT,
                                                       4,
                                                       std::vector<int>({1, 0, 0, 4}))),
                     testing::Values(MLUOP_UNKNOWN_DEVICE),
                     testing::Values(MLUOP_STATUS_BAD_PARAM)));

INSTANTIATE_TEST_CASE_P(
    negative_dtype_layout_shape_grad_output,
    border_align_backward_general,
    testing::Combine(
        testing::Values(
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC, MLUOP_DTYPE_HALF, 4, std::vector<int>({1, 4, 4, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_INT32,
                              4,
                              std::vector<int>({1, 4, 4, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NCHW,
                              MLUOP_DTYPE_FLOAT,
                              4,
                              std::vector<int>({1, 4, 4, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NLC, MLUOP_DTYPE_FLOAT, 4, std::vector<int>({1, 4, 4, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC, MLUOP_DTYPE_FLOAT, 3, std::vector<int>({1, 4, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_FLOAT,
                              5,
                              std::vector<int>({1, 4, 4, 1, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_FLOAT,
                              4,
                              std::vector<int>({2, 4, 4, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_FLOAT,
                              4,
                              std::vector<int>({1, 3, 4, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_FLOAT,
                              4,
                              std::vector<int>({1, 4, 3, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_FLOAT,
                              4,
                              std::vector<int>({1, 4, 4, 2}))),
        testing::Values(
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY, MLUOP_DTYPE_FLOAT, 3, std::vector<int>({1, 4, 4}))),
        testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                          MLUOP_DTYPE_INT32,
                                          4,
                                          std::vector<int>({1, 4, 4, 1}))),
...
INSTANTIATE_TEST_CASE_P(
    negative_dtype_layout_shape_boxes,
    border_align_backward_general,
    testing::Combine(
        testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                          MLUOP_DTYPE_FLOAT,
                                          ...
                                          std::vector<int>({1, 4, 4, 1}))),
        testing::Values(
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY, MLUOP_DTYPE_HALF, 3, std::vector<int>({1, 4, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY, MLUOP_DTYPE_INT32, 3, std::vector<int>({1, 4, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY, MLUOP_DTYPE_FLOAT, 2, std::vector<int>({1, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY,
                              MLUOP_DTYPE_FLOAT,
                              4,
                              std::vector<int>({1, 4, 4, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY, MLUOP_DTYPE_FLOAT, 3, std::vector<int>({2, 4, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY, MLUOP_DTYPE_FLOAT, 3, std::vector<int>({1, 3, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY, MLUOP_DTYPE_FLOAT, 3, std::vector<int>({1, 4, 3}))),
        testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                          MLUOP_DTYPE_INT32,
                                          4,
                                          std::vector<int>({1, 4, 4, 1}))),
...
INSTANTIATE_TEST_CASE_P(
    negative_dtype_layout_shape_argmax_idx,
    border_align_backward_general,
    testing::Combine(
        testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                          MLUOP_DTYPE_FLOAT,
        ...
        testing::Values(
            ...
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY,
                              ...
                              4,
                              std::vector<int>({1, 4, 4, 1})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC, MLUOP_DTYPE_INT32, 3, std::vector<int>({1, 4, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_INT32,
                              5,
                              std::vector<int>({1, 4, 4, 1, 1})),
...
INSTANTIATE_TEST_CASE_P(
    negative_dtype_layout_shape_grad_input,
    border_align_backward_general,
    testing::Combine(
        testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                          MLUOP_DTYPE_FLOAT,
        ...
        testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                          ...
                                          std::vector<int>({1, 4, 4, 1}))),
        testing::Values(
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC, MLUOP_DTYPE_HALF, 4, std::vector<int>({1, 2, 2, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_INT32,
                              4,
                              std::vector<int>({1, 2, 2, 4})),
            ...
            MLUOpTensorParam(MLUOP_LAYOUT_ARRAY,
                              ...
                              4,
                              std::vector<int>({1, 2, 2, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC, MLUOP_DTYPE_FLOAT, 3, std::vector<int>({1, 2, 2})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_FLOAT,
                              5,
                              std::vector<int>({1, 2, 2, 4, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_FLOAT,
                              4,
            ...
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_FLOAT,
                              4,
                              std::vector<int>({1, 1, 5, 4})),
            MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                              MLUOP_DTYPE_FLOAT,
                              4,
                              std::vector<int>({1, 2, 2, 5}))),
        testing::Values(MLUOP_UNKNOWN_DEVICE),
        testing::Values(MLUOP_STATUS_BAD_PARAM)));
...
INSTANTIATE_TEST_CASE_P(
    negative_layout,
    border_align_backward_general,
    testing::Combine(testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NCHW,
                                                       MLUOP_DTYPE_FLOAT,
                                                       4,
                                                       std::vector<int>({1, 4, 4, 1}))),
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_ARRAY,
                                                       ...
                                                       3,
                                                       std::vector<int>({1, 4, 4}))),
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NCHW,
                                                       MLUOP_DTYPE_INT32,
                                                       4,
                                                       std::vector<int>({1, 4, 4, 1}))),
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NCHW,
...
INSTANTIATE_TEST_CASE_P(
    ...
    testing::Combine(testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                     ...
                     testing::Values(MLUOpTensorParam(MLUOP_LAYOUT_NHWC,
                                                       MLUOP_DTYPE_HALF,
                                                       4,
                                                       std::vector<int>({1025, 32, 32, 2048}))),
                     testing::Values(MLUOP_UNKNOWN_DEVICE),
