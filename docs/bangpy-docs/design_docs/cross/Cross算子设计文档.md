...
| ---------- | ------------------------------------------------------------ | -------- | ---------- |
| 2022.06.01 | shape=(2,1024,2,4,3,2,8192,2)，以及其他维度相同时，任何维度大于该shape对应维度的情况 | 运行报错，很可能是规模太大 | 否         |
|            |                                                              |          |            |
...
当step>data_each_buffer时，shape规模超过一定范围后运行会出错（详见4.1)，很可能是规模太大，如果能在后续BANGPy版本下解决则加以解决。
