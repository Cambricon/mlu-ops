...
# mluoptest/mluoptest_database/mluop_to_sql_v2.py has the same enum
@enum.unique
...
MLU220、MLU220-EDGE、MLU270-X5K、MLU270、MLU290-M5、MLU290、MLU370-S4、MLU370、MLU370-X4、MLU370-X8、MLU365-D2、MLU270-F4、MLU590
...
parser.add_argument("--database", type=str, default=os.getenv("DATABASE", "mluop_dev"), help="database name to connect")
...
parser.add_argument("--network_table", type=str, default=os.getenv("NETWORK_TABLE", "mluop_network_list_test"), help="search case table to be queried from")
...
_db_table=args.table if args.table else "mluop_case_information"
_db_cvid=""
...
if args.branch=="master":
    _db_cvid=args.cvid if args.cvid else "mluop_cvID_information"
...
if _db_name=="training_solution":
        ...
        _db_host='10.101.9.21'
        _db_user='mluop_dev'
        _db_table=args.table if args.table else "mluop_case_information_benchmark_test"
        _db_cvid=args.cvid if args.cvid else "mluop_case_in_network_test"
