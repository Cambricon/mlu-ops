...
| **iou_thresh**   |   计算所需的阈值             | 输入              |    float                   | /       | 标量       |
...
- **nram 空间划分**
  ...
  nram_space暂划分为（COMPUTE_COUNT_ALIGN + 224*N）份，具体划分块数由计算overlap部分决定
...
由于公共模块的Convex-Hull-Graham排序顶点算法目前设计为标量循环实现，所以性能暂无估计，其他部分已做了向量化的优化。实际由于标量计算占比时间很大，会有额外的寄存器换入换出操作，以及额外的间接寻址时间，造成理论预估时间不准确。 
...
- 测试规模：boxes：(1-1000), iou_thresh：(0.0-1.0)；
...
当input1的boxes数量超过1000时，有超时风险, 故input1.shape[0]范围限制在[1,1000]。