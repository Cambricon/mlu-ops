...
`FocalLoss` 是在`CrossEntropyLoss` 的基础上增加了卷积滤波张量系数`alpha`和聚焦系数`gamma`，其目的是通过减少易分类样本的卷积滤波张量，从而使得模型在训练时更专注于难分类的样本。对于二分类问题：
```math
p = Sigmoid(input)
```
...
| weight    | 卷积滤波张量            | 输入              | float/half          |一维ARRAY   | shape为`[C]` |
