 ...
 */
enum QuantMode { ONLY_POSITION = 0, POSITION_SCALE = 1, POS_SCALE_OFFSET = 2, NO_QUANT = 3 };
...
class Executor {
 ...
 protected:
  ...
  void castDataIn(float *src_data,
  ...
  void castDataOut(void *src_data,
                   mluOpDataType_t src_dtype,
                   float *dst_data,
                   mluOpDataType_t dst_dtype,
                   size_t count,
 ...
 protected:
  void tensor_stride_in(void *dst,
                        void *src,
                        const std::vector<int> &shape,
                        const std::vector<int> &dst_stride,
                        size_t sizeof_dtype);
  void tensor_stride_out(void *dst,
                         void *src,
                         const std::vector<int> &shape,
