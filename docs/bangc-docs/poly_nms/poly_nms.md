...
6. copy所有的max_score对应的max_score_index到output，对output做升序排序；
   
7. 根据output中selected index数量来修改output_desc中dimSize的描述；
...
    __memcpy((char*)output  input_offset  loop  actual_box_num， (char)trans_boxes， actual_box_num * 4，
             NRAM2GDRAM);
  ...
}
