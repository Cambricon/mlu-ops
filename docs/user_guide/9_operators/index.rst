...
该算子包含8个输入：``handle`` 为操作句柄，``x_desc`` 与 ``x`` 分别描述并提供输入矩阵的信息，``y_desc`` 描述输出矩阵的信息，此外，还包含一个参数 ``mode``，用于指定是否进行选主元，值为0表示选择非主元模式，``dipiv_desc``表示置换矩阵的描述符，``dipiv``表示置换矩阵，以及一个 ``workspace`` 用于临时存储计算过程中的数据；两个输出：``y`` 存储输出矩阵的信息，``info``表示算法的执行状态；
