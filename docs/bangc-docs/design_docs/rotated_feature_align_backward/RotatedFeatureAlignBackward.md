...
本文档为 `RotatedFeatureAlignBackward` 算子的设计文档，包括需求分析、接口设计、方案设计、性能优化记录。该算子为 `RotatedFeatureAlignForward` 算子的反向算子。
...
图中，黄色点 P:(x，y) 为锚点框中心点坐标，由于 P 点不一定落在像素整数坐标位置上，因此需要计算 P 点邻近的四个整数像素点位置进行双线性插值计算 P 点的像素值。这里将 P 点的左上角位置 top_left 点的像素值定为 input[tl]，相应的加权系数为 w1 = v2 * u2；将 P 点的右上角位置 top_right 点的像素值定为 input[tr]，相应的加权系数为 w2 = v2 * u1；将 P 点的左下角位置 bottom_left 点的像素值定为 input[bl]，相应的加权系数为 w3 = v1 * u2；将 P 点的右下角位置 bottom_right 点的像素值定为 input[br]，相应的加权系数为 w4 = v1 * u1, 将Q 点在 input 上的像素值定位像素值定为 input[index]。这样可得到 Q 点像素值 valueq:
...
分别对 P、P1、P2、P3、P4 这五个点进行双线性插值计算每个点对应的像素值，以 P1 点为例子，如上图左侧，将 P1点的左上角位置 top_left_1 点的像素值定为 input[tl1]，相应的加权系数为 w11 = v21 * u21；将 P1 点的右上角位置 top_right_1 点的像素值定为 input[tr1]，相应的加权系数为 w21 = v21 * u11；将 P1 点的左下角位置 bottom_left_1 点的像素值定为 input[bl1]，相应的加权系数为 w31 = v11 * u21；将 P1 点的右下角位置 bottom_right_1 点的像素值定为 input[br1]，相应的加权系数为 w41 = v11 * u11。则可以得到 P1 点像素值 valuep1:
...
setp2: 根据 bboxes_ptr 中信息计算 P 点在 bottom_input 四领域的加权系数，分别为：w1、w2、w3、w4。
