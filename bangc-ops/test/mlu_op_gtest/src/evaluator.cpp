...
void resetNanOrInfAsZero(float *a, float *b, size_t count) {
  ...
  if (has_nan) {
    VLOG(4) << "Found result of baseline and mlu are both NaN, set them as 0, and go on.";
  }
  if (has_inf) {
    VLOG(4) << "Found result of baseline and mlu are both Inf, set them as 0, and go on.";
  }
}
...
double Evaluator::computeDiff1(float *cpu_result, float *mlu_result, size_t count) {
  if (hasNanOrInf(cpu_result, count) || hasNanOrInf(mlu_result, count)) {
    LOG(ERROR) << "Found NaN or Inf when compute diff, return DBL_MAX instead.";
    return DBL_MAX;
  }
...
double Evaluator::computeDiff2(float *cpu_result, float *mlu_result, size_t count) {
  if (hasNanOrInf(cpu_result, count) || hasNanOrInf(mlu_result, count)) {
    LOG(ERROR) << "Found NaN or Inf when compute diff, return DBL_MAX instead.";
    return DBL_MAX;
  }
...
// aka maxape
double Evaluator::computeDiff3(float *baseline_result,
                               float *mlu_result,
                               size_t count,
  ...
  for (int i = 0; i < count; ++i) {
    ...
    double ratio = 0;
    if (((MLUOP_DTYPE_HALF == dtype) && (fabs(baseline_result[i]) < EPSILON_HALF)) ||
        ((MLUOP_DTYPE_FLOAT == dtype) && (fabs(baseline_result[i]) < EPSILON_FLOAT))) {
      ratio = numerator;
    } else {
...
double Evaluator::computeDiff3_2(float *baseline_result, float *mlu_result, size_t count) {
  if (hasNanOrInf(baseline_result, count) || hasNanOrInf(mlu_result, count)) {
    LOG(ERROR) << "Found NaN or Inf when compute diff, return DBL_MAX instead.";
    return DBL_MAX;
  }
...
double Evaluator::computeDiff4(float *baseline_result, float *mlu_result, size_t count) {
  if (hasNanOrInf(baseline_result, count) || hasNanOrInf(mlu_result, count)) {
    LOG(ERROR) << "Found NaN or Inf when compute diff, return DBL_MAX instead.";
    return DBL_MAX;
  }
...
double Evaluator::computeError(float *baseline_result,
                               float *mlu_result,
                               size_t count,
  ...
  switch (func) {
    ...
    default:
      GTEST_CHECK(false, "Evaluator: found unsupported criterion when compute result error.");
  }
...
bool Evaluator::isPassed() {
  if (error_vec_.empty()) {
    LOG(WARNING) << "The result error is empty, it means output shape is 0 in pb, and skip compute "
                    "result error.";
  }
  ...
  for (int i = 0; i < error_vec_.size(); ++i) {
    ...
    if (std::isnan(error) || std::isinf(error)) {
      ...
      if (error > threshold || ((error < 1 - threshold) && (error != 0)) || error < 0) {
        return false;
      }
...
std::vector<std::string> Evaluator::what() {
  ...
  for (int i = 0; i < error_vec_.size(); ++i) {
    ...
    if (std::isnan(error) || std::isinf(error)) {
      ...
      oss << error;
      res.emplace_back("The error " + oss.str() + " of [" + name + "] is NOT digit.");
    } else if (Formula::DIFF4 == func) {
      if (error > threshold || ((error < 1 - threshold) && (error != 0)) || error < 0) {
        std::ostringstream oss;
        oss.setf(std::ios::scientific);
        oss << error;
        res.emplace_back("The error " + oss.str() + " of [" + name + "] is over " +
                         showFormula(func) + " threshold " + " [" + std::to_string(1 - threshold) +
      ...
      res.emplace_back("The error " + oss_error.str() + " of [" + name + "] is over " +
