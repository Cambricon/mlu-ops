...
|        | G-->N2 |  C(N2)  | N2-->S | G-->N2 |  C(N2) |           | N2-->S | G-->N2 | C(N2)   | N2-->S  | **S-->G** | G-->N2 | ... |
    ...
    (c)基于以上(a)和(b)两点的分析，第二个模型的性能很可能优于第一个模型。然后在之前的BANGPy版本中，由于for_range中的stage参数并不能实现上述逻辑，于是自己尝试手动实现了一下进行了验证。在不考虑结果是否正确的情况下，结果是第二个模型的性能却略低于第一个模型。下面是可能的原因：一方面，IO-DMA与Shared-DMA虽然是并行的，但二者会竞争NRAM的带宽；另一方面，引入SRAM会多出其它的通信等开销。并且二者虽然性能接近，但第二个逻辑要复杂一些，加上实现也较为困难，因此不再考虑。  
