...
```bash
#使能dump data
source env_dumpdata_set.sh on
#关闭dump data
source env_dumpdata_set.sh off
```
```bash
#使能gencase
source env_gencase_set.sh on
#关闭gencase
source env_gencase_set.sh off
```
...
|11|MLUOP_GTEST_SET_GDRAM|作用是在gdram前后刷NAN/INF| NAN/INF  在gdram前后刷NAN/INF|若不设置，偶数天刷NAN，奇数天刷INF|
