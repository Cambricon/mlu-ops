...
{   std::chrono::system_clock::time_point now = std::chrono::system_clock::now();   return std::chrono::system_clock::to_time_t(now); }

inline static std::string getCurrentTimeStr()
{   std::time_t current_time = getCurrentTimeT();   std::tm *local_time = std::localtime(&current_time);   char buffer[80];   std::strftime(buffer, sizeof(buffer), "%Y-%m-%d %H:%M:%S", local_time);   return std::string(buffer); }
...
inline static unsigned long getFileSize(const std::string &filename) {
  ...
  if (fd == -1)
{     std::cerr << "File open failed: " << filename << ". Reason: " << errno << "-"               << strerror(errno);     return 0;   }

  int ret_stat = fstat(fd, &file_stat);
  if (ret_stat == -1)
{     std::cerr << "File stat failed: " << filename << ". Reason: " << errno << "-"               << strerror(errno);     close(fd);     return 0;   }
