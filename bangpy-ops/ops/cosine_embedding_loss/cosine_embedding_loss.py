    ...
    def compute_body(self):
        ...
        with self.tcp.for_range(0, outer_loop_bound) as i:
            ...
            with self.tcp.for_range(0, inner_loop_bound, stage = self.pipeline) as j:
                ...
                with self.tcp.block("compute"):
                    ...
                    with self.tcp.else_scope():
                        ...
                        with self.tcp.if_scope(rows_per_line == 0):
                            comps_x1 = input_buffer_x1[: batch_size * self.length]\
                                .reshape((batch_size, self.length))
                            comps_x2 = input_buffer_x2[: batch_size * self.length]\
                                .reshape((batch_size, self.length))

                            comp_inter = inter_buffer\
                                [:kernels_per_line_n * self.align_size * kernel_size]\
                                .reshape((kernel_size * kernels_per_line_n, self.align_size, ))
                            temp_n = temp_buffer[:kernels_per_line_n * self.align_size]\
                            ...
                            with self.tcp.for_range(0, batch_size) as k:
                                ...
                                upper[k] = ((input_buffer_y[k] + 1) * (1 - upper_sum) +\
                                    (1 - input_buffer_y[k]) * self.tcp.scalar_max\
                                    (lower1_sum, lower2_sum)) / 2
                        ...
                        with self.tcp.else_scope():
                            ...
                            temp_n = temp_buffer[:kernels_per_row * batch_size]\
            ...
            with self.tcp.if_scope(batch_size == 0):
                ...
                lower1_sum.assign(0)
                lower2_sum.assign(upper_sum - self.margin)
                self.output[row] = ((self.input_y[row] + 1) *\
                    (1 - upper_sum) + (1 - self.input_y[row])\
