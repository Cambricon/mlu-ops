...
| ------  | ----------------------------- | ------------------| ----------- | -----------------| ------------------------------------------ |
| input   | 输入任意shape的张量            |       输入         | half, float |   \             | 每次输入张量所占空间大小小于MLUGDRAM的一半    |
...
loop:每个task需要拷入NRAM进行计算的次数(data_each_tsk // data_each_time)  
data_rem_n: 不足一次计算(data_each_tsk % data_each_time)
