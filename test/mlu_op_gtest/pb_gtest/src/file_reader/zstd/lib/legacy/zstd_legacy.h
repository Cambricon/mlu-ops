...
MEM_STATIC unsigned long long ZSTD_getDecompressedSize_legacy(const void* src,
                                                              size_t srcSize) {
...
MEM_STATIC size_t ZSTD_decompressLegacy(void* dst, size_t dstCapacity,
...
MEM_STATIC ZSTD_frameSizeInfo ZSTD_findFrameSizeInfoLegacy(const void* src,
                                                           size_t srcSize) {
...
MEM_STATIC size_t ZSTD_freeLegacyStreamContext(void* legacyContext,
                                               U32 version) {
  switch (version) {
    default:
    case 1:
...
MEM_STATIC size_t ZSTD_initLegacyStream(void** legacyContext, U32 prevVersion,
                                        U32 newVersion, const void* dict,
                                        size_t dictSize) {
  char x;
  /* Avoid passing NULL to legacy decoding. */
  if (dict == NULL) {
