...
| 输入 Layout                                                  | x: NCHW<br />img_size: ARRAY<br />anchors: ARRAY             |
...
2. **输入tensor说明**
   ...
   3. **输入anchors tensor**： shape为`[2*S]`，表示anchor的宽度和高度，计算时将逐对解析，其中S表示每个像素点应预测框的数量，数据类型int。
...
| boxes_desc       | 对输入数据 boxes 的形状描述，包含了 boxes 的数据类型、数据维度和布局等信息 | 输入              | mluOpTensorDescriptor_t | /        | 见1.4    |
| boxes            | 输出 tensor boxes 的地址，保存每个框的左上角和右下角的坐标   | 输出              | float                   | ARRAY    | 无       |
| scores_desc      | 对输入数据  scores 的形状描述，包含了 scores 的数据类型、数据维度和布局等信息 | 输入              | mluOpTensorDescriptor_t | /        | 见1.4    |
...
| 布局限制     | x为 NCHW <br />img_size、anchors、boxes、scores：ARRAY       |
| 规模限制     | anchors的shape为`[2*S]`，S须大于0<br />x的shape为`[N,C,H,W]`，C须大于0<br />img_size的shape为`[N, 2]`，dim[1]为2<br />boxes的shape为`[N, S, 4, H*W]`，dim[1]==S，dim[2]为4<br />scores的shape为`[N, S, class_num, H*W]`，dim[1]==S，dim[2]为class_num |
| 功能限制     | 无限制                                                       |
| 数据范围限制 | 无                                                           |
| 原位限制     | 不支持原位                                                   |
...
- C维度的前4个元素用来解码真实的bbox坐标，第5个元素置信度需要和后续每一个元素（分类得分）做乘积，得到所有分类的真实得分；![yolo_box](./res/score_pred.png)  
  ...
  其中cls[class_num]表示有class_num个分类得分。
  ...
  4. compute计算过程：
     ...
     3. 根据**公式（2）**、**公式（3）**，计算bbox的左上角`(x0,y0)`和右下角`(x1,y1)`坐标；
        ...
        1. 计算中心点坐标和宽高
           ...
           __bang_active_sigmoid(nram_y, nram_x, deal_num);
        ...
        2. 计算左上角和右下角坐标
           ...
           __bang_sub_scalar(nram_x, nram_x, nram_conf, deal_num);
           __bang_mul_scalar(nram_conf, nram_h, 0.5, deal_num);
           __bang_sub_scalar(nram_y, nram_y, nram_conf, deal_num);
           __bang_add_scalar(nram_w, nram_x, nram_w, deal_num);
           __bang_add_scalar(nram_h, nram_y, nram_h, deal_num);
           ```
        ...
        3. 坐标裁剪：当clip_bbox==true时，将bbox的左上角和右下角坐标裁剪到输入图像的宽img<sub>w</sub>和高img<sub>h</sub>
           ...
           __bang_maxeq_scalar(nram_y, nram_x, 0, deal_num);
