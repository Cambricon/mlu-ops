...
/**
  ...
  conversion(CC). This explicitly marks such functions as __cdecl so that the
  code will still compile if a CC other than __cdecl has been made the default.
*/
#if defined(_MSC_VER)
 ...
 * `mem.h` unit. Updating the prefix is probably preferable, but requires a
 * fairly large codemod, since this name is used everywhere.
 */
...
#ifndef STATIC_BMI2
#if defined(_MSC_VER) && (defined(_M_X64) || defined(_M_I86))
#ifdef __AVX2__  // MSVC does not have a BMI2 specific flag, but every CPU that
                 // supports AVX2 also supports BMI2
...
#else
/* older versions of clang [3.7, 5.0) will warn that pointer-overflow is
 * ignored. */
#define ZSTD_ALLOW_POINTER_OVERFLOW_ATTR \
...
MEM_STATIC
ZSTD_ALLOW_POINTER_OVERFLOW_ATTR
unsigned char const* ZSTD_wrappedPtrAdd(unsigned char const* ptr,
                                        ptrdiff_t add) {
  return ptr + add;
}
...
MEM_STATIC
ZSTD_ALLOW_POINTER_OVERFLOW_ATTR
unsigned char const* ZSTD_wrappedPtrSub(unsigned char const* ptr,
                                        ptrdiff_t sub) {
  return ptr - sub;
}
