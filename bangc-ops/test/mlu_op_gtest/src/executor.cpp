...
namespace mluoptest {
  ...
  EvaluateResult Executor::teardown() {
    ...
    if (exe_config_->perf_repeat > 1) {
      ...
      GTEST_CHECK(CNRT_RET_SUCCESS == cnrtPlaceNotifier(exe_context_->n_start,
                                                        exe_context_->queue));
  ...
  EvaluateResult Executor::evaluate() {
    ...
    for (int i = 0; i < getOutputBlocks().size(); ++i) {
      ...
      if (exe_config_->dump_data) {
        if (!ts->stride.empty()) {
          saveDataToFile("baseline_raw_" + ts->name,
                         cpu_fp32_output_[i],
                         getCpuDtype(ts->dtype),
                         ts->shape_count);
        }
      }
#endif
      ...
      if (!ts->stride.empty()) {
        ...
        tensor_stride_out(
            temp, cpu_fp32_output_[i], ts->shape, ts->stride, cpu_dtype_size);
      ...
      if (exe_config_->dump_data) {
        saveDataToFile("baseline_" + ts->name,
                       cpu_fp32_output_[i],
                       getCpuDtype(ts->dtype),
                       ts->total_count);
        saveDataToFile("mlu_" + ts->name,
                       mlu_fp32_output_[i],
                       getCpuDtype(ts->dtype),
                       ts->total_count);
      }
      ...
      for (auto it = criterions.begin(); it != criterions.end(); ++it) {
        eva_->computeError(cpu_fp32_output_[i],
                           mlu_fp32_output_[i],
                           ts->total_count,
                           *it,
                           ts->name,
  ...
  void Executor::getMluPerfInfo(PerfInfo *res) {
    ...
    res->io_efficiency =
        eva_->computeEfficiency(res->theory_io /* * 1000 * 1000*/,
                                res->hardware_time,
  ...
  void Executor::checkBaseline() {
    ...
    if (is_get_base_data) {
      is_baseline_pass = updateBaselineStrategy(hw_time_mean,
                                                scale_bound,
                                                threshold_absolute,
                                                threshold_relative,
                                                &hw_time_base);
      if (!is_baseline_pass) {
        ...
        LOG(ERROR) << "[Baseline]:hardware time of baseline is " << hw_time_base
                   << " (us).";
        LOG(ERROR) << "[Baseline]:hardware time of this test is "
                   << hw_time_mean << " (us).";
      }
  ...
  double Executor::getLtPeakComputeForce() {
    ...
    if (MLUOP_MLU220 == platform) {
      ...
      if (dtype1 == MLUOP_DTYPE_INT8 && dtype2 == MLUOP_DTYPE_INT8) {
        ...
        return LT_PEAK_INT8_INT8_COMPUTE_FORCE_270_290 * cluster_num * core_num;
                 ...
                 (dtype1 == MLUOP_DTYPE_INT16 && dtype2 == MLUOP_DTYPE_INT8)) {
        // int16 + int8
        return LT_PEAK_INT16_INT8_COMPUTE_FORCE_270_290 * cluster_num *
               core_num;
      } else if (dtype1 == MLUOP_DTYPE_INT16 && dtype2 == MLUOP_DTYPE_INT16) {
        // int16 + int16
        return LT_PEAK_INT16_INT16_COMPUTE_FORCE_270_290 * cluster_num *
               core_num;
      } else if (dtype1 == MLUOP_DTYPE_INT31 || dtype2 == MLUOP_DTYPE_INT31) {
        // int31
    ...
    LOG(WARNING)
        << "Executor: got unsupported arch when get peak compute force.";
    return -1;
  }
  ...
  int64_t Executor::getTheoryIoSize() {
    size_t total_size = 0;
    std::for_each(data_vector_.begin(), data_vector_.end(), [&](DataBlock b) {
      total_size += b.size;
    });
  ...
  void Executor::createTensors() {
    auto create_tensor = [&](MetaTensor *mt, bool is_output) {
      ...
      if (mt->stride.empty()) { // if no stride testing this api.
        ...
        MLUOP_CHECK(mluOpSetTensorDescriptorEx(desc,
                                               mt->layout,
                                               mt->dtype,
                                               mt->shape.size(),
                                               mt->shape.data(),
  ...
  void Executor::initHostData() {
    for (size_t i = 0; i < parser_->inputs().size(); ++i) {
      ...
      parser_->getInputTensorValue(
          i, data_vector_[i].host_ptr, data_vector_[i].count);
    }
  }
    ...
    for (size_t i = 0; i < parser_->inputs().size(); ++i) {
      ...
      if (needZeroInput()) {
        ...
        VLOG(4) << "input data have been set zero";
                 ...
                 VALUE_PATH ==
                     ts->value_type) { // generate random or read from path
  ...
  void Executor::deviceMalloc() {
    ...
    if (flag_input_reuse_) {
      GTEST_CHECK(parser_->inputs().size(),
                  data_vector_.size(),
    ...
    *scale    = 1.0f;
    ...
    if (MLUOP_DTYPE_INT8 == dst_dtype || MLUOP_DTYPE_INT16 == dst_dtype) {
      ...
      if (ONLY_POSITION == quant_mode) {
        ...
        MLUOP_CHECK(getPositionScaleAndOffset(
            src_data, count, dst_dtype, position, scale, offset));
  ...
  void Executor::castDataIn(float *src_data,
    ...
    if (src_dtype == MLUOP_DTYPE_FLOAT && dst_dtype == MLUOP_DTYPE_FLOAT) {
      ...
      *offset = 0;
      getQuantizedParam(
          src_data, count, dst_dtype, quant_mode, pos, scale, offset);
      if (quant_mode != POS_SCALE_OFFSET) {
        ...
        GTEST_CHECK(
            CNRT_RET_SUCCESS ==
            cnrtCastDataType(
                src_data, in_dtype, dst_data, out_dtype, count, quant_param));
        if (dequantify) {
          GTEST_CHECK(
              CNRT_RET_SUCCESS ==
              cnrtCastDataType(
                  dst_data, out_dtype, src_data, in_dtype, count, quant_param));
        }
        GTEST_CHECK(CNRT_RET_SUCCESS == cnrtDestroyQuantizedParam(quant_param));
      } else {
        if (dst_dtype == MLUOP_DTYPE_INT8) {
          MLUOP_CHECK(castFloat32ToFixed(
              src_data, (int8_t *)dst_data, count, *pos, *scale, *offset));
          if (dequantify) {
            MLUOP_CHECK(castFixedToFloat32(
                (int8_t *)dst_data, src_data, count, *pos, *scale, *offset));
          }
        } else if (dst_dtype == MLUOP_DTYPE_INT16) {
          MLUOP_CHECK(castFloat32ToFixed(
              src_data, (int16_t *)dst_data, count, *pos, *scale, *offset));
          if (dequantify) {
            MLUOP_CHECK(castFixedToFloat32(
                (int16_t *)dst_data, src_data, count, *pos, *scale, *offset));
          }
        }
      }
    } else if ((src_dtype == MLUOP_DTYPE_FLOAT &&
    ...
    if (src_dtype == dst_dtype) {
      ...
      if (flag_quant_mode_ != POS_SCALE_OFFSET) {
        ...
        if (src_dtype == MLUOP_DTYPE_INT8) {
          MLUOP_CHECK(castFixedToFloat32(
              (int8_t *)src_data, dst_data, count, pos, scale, offset));
        } else if (src_dtype == MLUOP_DTYPE_INT16) {
          MLUOP_CHECK(castFixedToFloat32(
              (int16_t *)src_data, dst_data, count, pos, scale, offset));
        }
      }
    } else if (dst_dtype == MLUOP_DTYPE_FLOAT &&
               (src_dtype == MLUOP_DTYPE_HALF ||
                src_dtype == MLUOP_DTYPE_BOOL ||
      ...
      GTEST_CHECK(CNRT_RET_SUCCESS ==
                  cnrtCastDataType(
                      src_data, in_dtype, dst_data, out_dtype, count, nullptr));
    } else if (src_dtype == MLUOP_DTYPE_INT31 &&
               dst_dtype == MLUOP_DTYPE_FLOAT) {
      ...
      GTEST_CHECK(
          false, "Executor: when cast dtype to fp32, found unsupported dtype.");
    }
  }
  ...
  void Executor::castIn() {
    for (size_t i = 0; i < parser_->inputs().size(); ++i) {
      ...
      if (!ts->stride.empty()) {
        ...
        if (flag_input_reuse_) {
          memcpy(cpu_fp32_stride_input_[i],
                 cpu_fp32_input_[i],
                 ts->total_count * cpu_dtype_size);
        }
        tensor_stride_in(
            temp, cpu_fp32_input_[i], ts->shape, ts->stride, cpu_dtype_size);
        cpu_runtime_.deallocate(cpu_fp32_input_[i]);
  ...
  void Executor::copyIn() {
    for (size_t i = 0; i < getInputBlocks().size(); ++i) {
      ...
      if (exe_config_->dump_data) {
        saveHexDataToFile(
            "hex_" + db->name, db->host_ptr, db->dtype, db->count);
      }
    }
    ...
    if (d == shape.size() - 1) { // the last dim
      ...
      for (size_t i = 0; i < shape[d]; ++i) {
        stride_map(dst,
                   ...
                   dst_stride,
                   src_stride,
                   dst_offset + i * dst_stride[d],
                   src_offset + i * src_stride[d],
                   d + 1,
                   sizeof_dtype,
                   dst_max,
                   src_max);
  ...
  void Executor::tensor_stride_in(
    ...
    size_t shape_total = std::accumulate(
        shape.begin(), shape.end(), (int)1, std::multiplies<int>());
  ...
  void Executor::clusterLimitCheck() {
    ...
    if (set_cluster_num_ptr) {
      ...
      uint32_t union_number =
          mluop::runtime::getClusterLimitCapability(handle_);
  ...
  void Executor::setClusterLimitCapability(uint32_t cluster_limit) {
    ...
    int ret;
    ret = cnSetCtxConfigParam(
        ctx, CN_CTX_CONFIG_VISIBLE_CLUSTER, &ctx_conf_param);
    if (!ret) {
