...
| grad_gates      | 输出数据 grad_gates 的指针                                   | 输出              | half, float             | ARRAY    | 无       |
...
- 对于 samples <= taskDim 的 case，采用三级流水设计，L C S 之间排流水，即 GDRAM2NRAM、Compute、NRAM2GDRAM。
...
- 算子在网络中用到的规模：
  ...
  ```c
  int型变量：samples: 8192 ， capacity: 8192， hidden: 2048
  tensor：grad_gates: [8192,2]，float；
          indices: [8192]，int类型
...
- 边界 case：
  ...
  ```
  case1:
  int型变量：samples: 0 ， capacity: 8192， hidden: 2048
  tensor：grad_gates: [0,2]，float；
          indices: [0]，int类型
          locations: [0], int类型
          input: [0, 2048]，float类型
          dispatch: [0, 2048]， float类型
  case2:
  int型变量：samples: 8192 ， capacity: 8192， hidden: 0
  tensor：grad_gates: [8192,2]，float；
          indices: [8192]，int类型
          locations: [8192], int类型
...
- 对输入输出支持的 dtype、layout 以及 shape 进行防呆
  1. dtype防呆：input_desc、dispatch_desc、grad_gates_desc：仅支持half\float；
