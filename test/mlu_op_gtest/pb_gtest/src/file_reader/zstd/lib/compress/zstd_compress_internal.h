...
#define ZSTD_DUBT_UNSORTED_MARK                                     \
  1 /* For btlazy2 strategy, index ZSTD_DUBT_UNSORTED_MARK==1 means \
       ...
       cannot be mishandled after table reuse with a different strategy. This                                                               \
...
typedef struct {
  ...
  size_t posInSequence; /* The position within the sequence at seq[pos] where
                           reading stopped. posInSequence <= seq[pos].litLength
                           + seq[pos].matchLength */
...
typedef struct {
  ...
  U32 nbOverflowCorrections; /* Number of times overflow correction has run
                              * since ZSTD_window_init(). Useful for debugging
                              * coredumps and for
...
#define ZSTD_ROW_HASH_CACHE_SIZE \
  8 /* Size of prefetching hash cache for row-based matchfinder */
...
struct ZSTD_matchState_t {
  ...
  U32 rowHashLog; /* For row-based matchfinder: Hashlog based on nb of rows in
                     the hashTable.*/
  BYTE* tagTable; /* For row-based matchFinder: A row-based table containing the
                     hashes and head index. */
  U32 hashCache[ZSTD_ROW_HASH_CACHE_SIZE]; /* For row-based matchFinder: a cache
                                              of hashes to improve speed */
  U64 hashSalt; /* For row-based matchFinder: salts the hash for reuse of tag
                   table */
  U32 hashSaltEntropy; /* For row-based matchFinder: collects entropy for salt
                          generation */
...
typedef struct {
  ZSTD_paramSwitch_e
      enableLdm;     /* ZSTD_ps_enable to enable LDM. ZSTD_ps_auto by default */
...
struct ZSTD_CCtx_params_s {
   ...
   * It is not possible to set these parameters individually through the public
   * API. */
...
struct ZSTD_CCtx_s {
  ...
  ZSTD_CCtx_params
      simpleApiParams; /* Param storage used by the simple API - not sticky.
                          Must only be used in top-level simple API functions
                          for storage. */
  U32 dictID;
  ...
  size_t stableIn_notConsumed; /* nb bytes within stable input buffer that are
                                  said to be consumed but are not */
...
typedef enum {
  ...
  ZSTD_cpm_unknown =
      3 /* ZSTD_getCParams, ZSTD_getParams, ZSTD_adjustParams.
         ...
         * behavior of taking both the source size and the dict size into
         * account when selecting and adjusting parameters.
         */
} ZSTD_cParamMode_e;
...
ZSTD_blockCompressor ZSTD_selectBlockCompressor(
    ZSTD_strategy strat, ZSTD_paramSwitch_e rowMatchfinderMode,
...
MEM_STATIC size_t ZSTD_noCompressBlock(void* dst, size_t dstCapacity,
  ...
  DEBUGLOG(5, "ZSTD_noCompressBlock (srcSize=%zu, dstCapacity=%zu)", srcSize,
           dstCapacity);
 ...
 *  @offBase : Users should employ macros REPCODE_TO_OFFBASE() and
 * OFFSET_TO_OFFBASE().
...
HINT_INLINE UNUSED_ATTR void ZSTD_storeSeq(seqStore_t* seqStorePtr,
                                           size_t litLength,
                                           const BYTE* literals,
                                           const BYTE* litLimit, U32 offBase,
                                           size_t matchLength) {
                                        ...
                                        within a single segment */
  {
    ...
    DEBUGLOG(6, "Cpos%7u :%3u literals, match%4u bytes at offBase%7u", pos,
             (U32)litLength, (U32)matchLength, (U32)offBase);
  }
#endif
 ...
 */
MEM_STATIC void ZSTD_updateRep(U32 rep[ZSTD_REP_NUM], U32 const offBase,
                               U32 const ll0) {
...
MEM_STATIC repcodes_t ZSTD_newRep(U32 const rep[ZSTD_REP_NUM],
                                  U32 const offBase, U32 const ll0) {
  repcodes_t newReps;
...
MEM_STATIC FORCE_INLINE_ATTR size_t ZSTD_hashPtr(const void* p, U32 hBits,
                                                 U32 mls) {
...
MEM_STATIC FORCE_INLINE_ATTR size_t ZSTD_hashPtrSalted(const void* p, U32 hBits,
                                                       U32 mls,
                                                       const U64 hashSalt) {
  /* Although some of these hashes do support hBits up to 64, some do not.
   * To be on the safe side, always avoid hBits > 32. */
...
U32 ZSTD_window_correctOverflow(ZSTD_window_t* window, U32 cycleLog,
  ...
  U32 const currentCycleCorrection =
      currentCycle < ZSTD_WINDOW_START_INDEX
          ? MAX(cycleSize, ZSTD_WINDOW_START_INDEX)
          : 0;
  U32 const newCurrent =
      currentCycle + currentCycleCorrection + MAX(maxDist, cycleSize);
...
MEM_STATIC void ZSTD_checkDictValidity(
  ...
  assert(dictMatchStatePtr != NULL);
  {
    ...
    if (blockEndIdx > loadedDictEnd + maxDist ||
       ...
       *
       * We also have to invalidate the dictionary if ZSTD_window_update() has
       * detected non-contiguous segments, which means that loadedDictEnd !=
       * window->dictLimit. loadedDictEnd may be 0, if forceWindow is true, but
       * in that case we never use dictMatchState, so setting it to NULL is not
       * a problem.
       */
      DEBUGLOG(
          6,
...
MEM_STATIC void ZSTD_window_init(ZSTD_window_t* window) {
  ...
  window->dictLimit = ZSTD_WINDOW_START_INDEX; /* start from >0, so that 1st
                                                  position is valid */
  window->lowLimit = ZSTD_WINDOW_START_INDEX; /* it ensures first and later CCtx
                                                 usages compress the same */
  window->nextSrc =
...
MEM_STATIC
ZSTD_ALLOW_POINTER_OVERFLOW_ATTR
U32 ZSTD_window_update(ZSTD_window_t* window, void const* src, size_t srcSize,
...
MEM_STATIC U32 ZSTD_getLowestMatchIndex(const ZSTD_matchState_t* ms, U32 curr,
   ...
   * the dictionary is within the window. We invalidate the dictionary (and set
   * loadedDictEnd to 0) when it isn't valid for the entire block. So this check
   * is sufficient to find the lowest valid match index.
   */
...
MEM_STATIC U32 ZSTD_getLowestPrefixIndex(const ZSTD_matchState_t* ms, U32 curr,
   ...
   * account to handle the edge case where the dictionary and the source are
 ...
 * In dictionary compression, loading *(base + index) is often an L2 or even L3
 * miss.
 *
 * Short cache is an optimization which allows us to avoid step 3 most of the
 * time when the data doesn't actually match. With short cache, the flow
 * becomes:
 *     1. Compute (hash, currentTag) at ip. currentTag is an 8-bit independent
 * hash at ip.
 *     2. Load (index, matchTag) from hashTable[hash]. See ZSTD_writeTaggedIndex
 * to understand how this works.
 *     3. Only if currentTag == matchTag, check *ip == *(base + index).
 * Otherwise, continue.
 *
 * Currently, short cache is only implemented in CDict hashtables. Thus, its use
 * is limited to dictMatchState matchfinders.
 */
#define ZSTD_SHORT_CACHE_TAG_BITS 8
#define ZSTD_SHORT_CACHE_TAG_MASK ((1u << ZSTD_SHORT_CACHE_TAG_BITS) - 1)

/* Helper function for ZSTD_fillHashTable and ZSTD_fillDoubleHashTable.
 * Unpacks hashAndTag into (hash, tag), then packs (index, tag) into
 * hashTable[hash]. */
MEM_STATIC void ZSTD_writeTaggedIndex(U32* const hashTable, size_t hashAndTag,
                                      U32 index) {
 ...
 * Unpacks tag1 and tag2 from lower bits of packedTag1 and packedTag2, then
 ...
 * an array of ZSTD_Sequence, storing the sequences it finds, until it reaches a
 * block delimiter. Note that the block delimiter must include the last literals
 * of the block.
 */
 ...
 * from inSeqs to avoid splitting a match, or to avoid splitting a match such
 * that it would produce a match smaller than MINMATCH. In this case, we return
 * the number of bytes that we didn't read from this block.
 */
...
/* Returns 1 if an external sequence producer is registered, otherwise returns
 * 0. */
