...
mluOpStatus_t MLUOP_WIN_API mluOpMlNms(mluOpHandle_t handle,
    ...
    void* scores_max_boxes_data_ptr, int input_boxes_num, float iou_threshold,
    ...
    mluOpStatus_t param_check = unaryOpNmsParamCheck(
      "[mluOpMlNms]", boxes_data_ptr_desc, boxes_data_ptr,
      scores_max_boxes_data_ptr, support_type, 2, zero_element);
    ...
    void (*mluOpFuncKernel)(cnrtDim3_t k_dim, cnrtFunctionType_t k_type,
      ...
      void* scores_max_boxes_data_ptr, float nmsThres, int input_boxes_num,
      uint8_t* output_boxes_index);
    ...
    KERNEL_CHECK(
      (mluOpFuncKernel(k_dim, k_type, handle->queue, boxes_data_ptr_desc->dtype,
          boxes_data_ptr, scores_max_boxes_data_ptr, iou_threshold,
