...
![](https://latex.codecogs.com/svg.image?Hardshrink(x)=\begin{cases}&space;x,&x%3E\lambda&space;\\&space;x,&x%3C-\lambda&space;\\&space;0,&otherwise&space;\end{cases})
...
```python
tensor_in = bp.Tensor(shape,name = "INPUT", dtype = dtype, scope = "global")
tensor_out = bp.Tensor(shape,name = "OUTPUT" ,dtype = dtype, scope = "global")
scalar_lambda = bp.Scalar(value,scope = "global")

tensor_in_n = bp.Tensor(shape,name="INPUT_N",dtype = dtype, scope = "nram")
tensor_out_n = bp.Tensor(shape,name = "OUTPUT" ,dtype = dtype, scope = "nram")
scalar_lambda_n = bp.Scalar(value,scope = "nram")

memcpy(gloabl,nram)

tensor_temp_abs = bp.Tensor(scope = "nram")
