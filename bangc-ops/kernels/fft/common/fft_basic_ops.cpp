...
mluOpStatus_t fftQuantizePositionScale(mluOpHandle_t handle,
                                      int array_length,
                                      mluOpDataType_t data_type,
                                      mluOpDataType_t compute_type,
                                      const void *input,
...
mluOpStatus_t fftGetQuantizeMatMulWorkspaceSize(mluOpHandle_t handle,
                                               size_t& workspace_size,
                                               int m,
                                               int k,
                                               int n,
  ...
  status = mluOpSetTensorDescriptor_v2(a_desc, MLUOP_LAYOUT_ARRAY, data_type, 2, a_dims);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  status = mluOpSetTensorDescriptorOnchipDataType(a_desc, a_compute_type);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  status = mluOpSetTensorDescriptor_v2(b_desc, MLUOP_LAYOUT_ARRAY, data_type, 2, b_dims);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  status = mluOpSetTensorDescriptorOnchipDataType(b_desc, b_compute_type);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  status = mluOpSetTensorDescriptor_v2(c_desc, MLUOP_LAYOUT_ARRAY, data_type, 2, c_dims);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  if (a_compute_type == MLUOP_DTYPE_INT31 || b_compute_type == MLUOP_DTYPE_INT31) {
...
mluOpStatus_t fftQuantMatMul(mluOpHandle_t handle,
                            int m,
                            int k,
                            int n,
                            void *a_ptr,
...
mluOpStatus_t fftBatchMatMulBcast(mluOpHandle_t handle,
                                 int m,  // 2 * L = 750
                                 int k,  // L = 375
                                 int n,  // 2^m = 128
                                 int batch,
  ...
  status = mluOpSetTensorDescriptor(a_desc, MLUOP_LAYOUT_ARRAY, data_type, 2, a_dims);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  status = mluOpSetTensorDescriptorOnchipDataType(a_desc, a_compute_type);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  status = mluOpSetTensorDescriptor(b_desc, MLUOP_LAYOUT_ARRAY, data_type, 3, b_dims);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  status = mluOpSetTensorDescriptorOnchipDataType(b_desc, b_compute_type);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
...
mluOpStatus_t fftTranspose(mluOpHandle_t handle,
                          int dim_num,
                          int ori_dims[],
                          int transed_dims[],
                          int permute[],
  ...
  status = mluOpSetTensorDescriptor(input_desc, MLUOP_LAYOUT_ARRAY, data_type, dim_num, ori_dims);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  status = mluOpSetTensorDescriptor(transed_input_desc, MLUOP_LAYOUT_ARRAY, data_type, dim_num,
                                   transed_dims);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
...
mluOpStatus_t fftGetOptensorWorkspaceSize(mluOpHandle_t handle,
  ...
  status = mluOpSetTensorDescriptor_v2(in1_desc, MLUOP_LAYOUT_ARRAY, data_type, 1, dims);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
  status = mluOpSetTensorDescriptor_v2(in2_desc, MLUOP_LAYOUT_ARRAY, data_type, 1, dims);
  INTERNAL_CHECK(api, status == MLUOP_STATUS_SUCCESS);
...
mluOpStatus_t fftOptensor(mluOpHandle_t handle,
                         int elem_num,
                         void *in1_ptr,
                         void *in2_ptr,
                         void *out_ptr,
...
bool findStockham(mluOpHandle_t handle, int& L, int& m, int& L_sub, bool& find_stockham) {
  if (find_stockham) {
    ...
    // 1 represents a public space stores the incremental sequence shared by ping_pong.
    int cal_unit_tmp = 14 * 2 + 1;
    size_t cal_unit_once_tmp = L_sub * pow(2, m_tmp - 1) * cal_unit_tmp * sizeof(float);
    while (cal_unit_once_tmp > max_nram_size) {
      ...
      cal_unit_once_tmp = L_sub * pow(2, m_tmp - 1) * cal_unit_tmp * sizeof(float);
    }
...
bool findCooleyTukey(mluOpHandle_t handle, int& L, int& m, int& s) {
  int cal_unit = 14;
  int cal_unit_once = PAD_UP(L, NFU_ALIGN_SIZE / sizeof(float)) * cal_unit * sizeof(float);
...
int findFFTOptLimit(mluOpHandle_t handle, const int n, int& m, int& L, int& s, int& L_sub,
                    bool& find_stockham) {
