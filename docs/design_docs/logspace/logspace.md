...
| ---------------- | ---------------------------------- | ----------------- | ----------- | ---------- | -------- |
| handle           | 当前可获得的资源息                    | 输入                 | mluOpHandle_t           | /          | 无       |
...
2. 如果步数 `step` 大于1，首先计算本core需要处理的元素数量，具体见3.3拆分。
...
- 边界case：
  ...
  其他可根据需要进行补充。算子开发完毕后，补充测试报告，目前手工测试该算子，性能在v100的1倍左右，前四种长度性能优于v100，[262144]长度性能差于v100。
