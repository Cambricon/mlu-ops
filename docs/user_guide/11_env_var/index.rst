...
默认不设置该环境变量。不再使用此环境变量时，需要unset环境变量，或者设置为OFF。
...
- export MLUOP_SET_JOB_LIMIT_CAPABILITY=1：CN_KERNEL_CLASS_UNION1。
...
JOB_LIMIT和CLUSTER_LIMIT需要同时设置来保证合法性，CN_KERNEL_CLASS_UNION代表任务类型。

.. _MLUOP_SET_CLUSTER_LIMIT_CAPABILITY:

MLUOP_SET_CLUSTER_LIMIT_CAPABILITY
######################################
...
- export MLUOP_SET_CLUSTER_LIMIT_CAPABILITY=1：1cluster。
- export MLUOP_SET_CLUSTER_LIMIT_CAPABILITY=3：2cluster。
- export MLUOP_SET_CLUSTER_LIMIT_CAPABILITY=7：3cluster。
- export MLUOP_SET_CLUSTER_LIMIT_CAPABILITY=15：4cluster。
- export MLUOP_SET_CLUSTER_LIMIT_CAPABILITY=...：从右往左，每多一个连续的1表示1个cluster。
